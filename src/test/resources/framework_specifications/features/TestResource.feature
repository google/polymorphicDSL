Ability: Test Resources
	...
	A test resource is any data that can be represented by a URI that can later be converted into a Test Specification. These are commonly text files, but they can also be arbitrary resources on the internet, Java Archives or anything else suppored by a URI encoding scheme.
	...
	Rule: Any valid URI is technically a valid test resource. Some corresponding test specification factory will attempt to open the resource and will raise an error in the event of a problem (file doesn't exist, internet offline, etc).
