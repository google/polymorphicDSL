    ...
    protected void runChild(final FrameworkMethod method, RunNotifier notifier) {
        ...
        if (pdslTest != null) {
            ...
            if (isIgnored(method)) {
                ...
                final ParseTreeListener parseTreeListener = getParseTreeListener(pdslTest);
                ...
                PdslStatement statement = new PdslStatement(testCases, parseTreeListener, context, executor);
                runLeaf(statement, description, notifier);
                Optional<MetadataTestRunResults> runResults = statement.getResults();
                if (runResults.isPresent()) {
                    results.add(statement.getResults().orElseThrow());
                } else {
